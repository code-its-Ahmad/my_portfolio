...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Sending contact request:', payload); // Debugging
  
      const response = await fetch('http://localhost:8000/api/contact', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Origin': 'https://m-ahmad-portfolio-dev.netlify.app', // Explicitly set Origin
        },
        body: JSON.stringify(payload),
      ...
      toast.error(
        `Failed to send message: ${error.message}. Please try again or contact Muhammad directly at Ahmadrajpootr1@gmail.com or +923314815161.`,
        { duration: 6000 }
      );
    } finally {
      setIsSubmitting(false);
